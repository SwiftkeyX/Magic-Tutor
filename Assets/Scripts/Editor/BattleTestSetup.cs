...
/// One-shot editor setup script for the BattleTest scene.
...
public class BattleTestSetup
{
    public static void Execute()
    {
        ...
        // ── 5. BattleTestController GameObject ───────────────────────────
        var controllerGO = new GameObject("BattleTestController");
        var controller = controllerGO.AddComponent<MagicSchool.Battle.BattleTestController>();

        // Wire prefabs via SerializedObject
        ...
        so.FindProperty("hexTilePrefab").objectReferenceValue  = tilePrefab;
        so.FindProperty("battleUnitPrefab").objectReferenceValue = unitPrefab;
        ...
        Debug.Log("[BattleTestSetup] Press Play to run the battle test!");
    }
